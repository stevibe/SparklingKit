...
import { Check, CheckCircle2, Clock3, FileImage, FileText, LoaderCircle, Mic2, Pencil, Trash2, TriangleAlert, XCircle } from "lucide-react";
import type { Job, JobKind, JobStatus } from "../types";

export function cn(...inputs: ClassValue[]) { return twMerge(clsx(inputs)); }

export function JobIcon({ type, className }: { type: JobKind; className?: string }) {
  const Icon = type === "audio" ? Mic2 : type === "image" || type === "text" ? FileImage : FileText;
  return <span className={cn("job-icon", `job-icon-${type}`, className)}><Icon size={20} strokeWidth={1.8} /></span>;
}
