...
describe("module artifact router", () => {
  it("routes generated images into every compatible image workflow", () => {
    ...
    expect(compatibleModuleContracts("generated-image", "text-to-image", ["text", "image"]).map((contract) => contract.id)).toEqual(["ocr", "grounding", "chat"]);
  ...
  it("routes text results into translation, image generation, and chat", () => {
    expect(compatibleModuleContracts("document", "ocr").map((contract) => contract.id)).toEqual(["translation", "text-to-image", "chat"]);
  ...
  it("derives outgoing actions and handoff URLs from the same contract", () => {
    ...
    expect(nextModuleActions(ocr).map((action) => action.id)).toEqual(["translation", "text-to-image", "chat"]);
    const imageGenerator = getModuleContract("text-to-image")!;
    expect(nextModuleActions(imageGenerator, ["text", "image"]).map((action) => action.id)).toEqual(["ocr", "grounding", "chat"]);
